...
//    protected $dataLimit = 'personal';
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\Square;
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            if($this->auth->isSuperAdmin() || $group_text  == '商户'){
                ...
                $query = Db::table('fa_mer_ven')->field('account')->where(['type'=>'Square','ven_id'=>$this->auth->id])->select();
...
//            exit();
    ...
    public function add()
    {
        ...
        if(empty($params['fees'])){
            $params['fees'] =$square_fees['square_fees'];
        }else{
    ...
    public function venderedit($ids = null){
        ...
        $zelle_quantity =Db::table('fa_square')->where('order_num',$params['order_num'])->field('quantity')->find();
    ...
    public function invoice($order_num)
    {
        ...
        if (empty($data['vendor_invoice'])) {
            $invoce = request()->domain()."/sFGucSHOMQ.php/square/invoice.html?order_num=".$data['order_num'];
        ...
        return $this->view->fetch('square/invoice');
    }
